...
if [ $1 = "debug" ];
then
    ros2 launch launch_package launch_drivers.launch.py use_fake_hardware:=false robot_ip:=192.168.125.12 -d -a; # Debug level + all subprocesses.
else 
    ros2 launch launch_package launch_drivers.launch.py use_fake_hardware:=false robot_ip:=192.168.125.12;
fi # Normal.